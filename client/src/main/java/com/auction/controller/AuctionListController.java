...
public class AuctionListController {
  ...
  private static final DateTimeFormatter BAN_EXPIRY_FMT =
  ...
  private String formatPrice(double amount) {
    if (amount >= 1_000_000_000) return String.format("$%.3gB", amount / 1_000_000_000);
    if (amount >= 1_000_000) return String.format("$%.3gM", amount / 1_000_000);
    if (amount >= 1_000) return String.format("$%.3gK", amount / 1_000);
    ...
    itemTable.setRowFactory(
        tv -> {
          ...
          row.setOnMouseClicked(
              e -> {
                if (e.getClickCount() == 2 && !row.isEmpty()) showItemDetailsCard(row.getItem());
              });
          return row;
        });
    itemTable
        ...
        .addListener((obs, oldItem, newItem) -> updatePaySellerButton());
    colPrice.setCellValueFactory(
        c -> new SimpleStringProperty(formatPrice(c.getValue().getCurrentPrice())));
  ...
  private void onAddFunds() {
    PaymentDialogFactory.showAddFundsDialog()
        .ifPresent(
            amount -> {
              try {
                ...
                User updated =
                    appState.restUserService.addBalance(appState.currentUser.getId(), amount);
  ...
  private void onWithdraw() {
    double committed =
        appState.itemRepository.findAll().stream()
            .filter(
                i ->
                    (i.getStatus() == AuctionStatus.RUNNING
                            || i.getStatus() == AuctionStatus.FINISHED)
                        && appState.currentUser.getId().equals(i.getCurrentWinnerId()))
            .mapToDouble(Item::getCurrentPrice)
            .sum();
    if (committed > 0) {
      showStatus(
          "Cannot withdraw while you have active winning bids ($"
        ...
        .ifPresent(
            val -> {
              try {
                ...
                User updated =
                    appState.restUserService.deductBalance(appState.currentUser.getId(), amount);
                appState.currentUser = updated;
                refreshUserInfo();
                showStatus(
                    "Withdrew $" + String.format("%.2f", amount) + " from your balance.", false);
              } catch (Exception e) {
                showStatus(e.getMessage(), true);
              }
            });
  ...
  private void onCreateItem() {
    ItemDialogFactory.build(appState, stage, null)
        .showAndWait()
        .ifPresent(
            item -> {
              try {
  ...
  private void onEditItem() {
    ...
    ItemDialogFactory.build(appState, stage, selected)
        .showAndWait()
        .ifPresent(
            item -> {
              try {
  ...
  private void onStart() {
    withSelected(
        item -> {
          appState.auctionService.startAuction(item.getId(), appState.currentUser);
          refreshTable();
          showStatus("Auction started.", false);
        });
  }

  @FXML
  private void onEndEarly() {
    withSelected(
        item -> {
          appState.auctionService.endAuctionEarly(item.getId(), appState.currentUser);
          refreshCurrentUser();
          refreshTable();
          showStatus("Auction ended early.", false);
        });
  }
  ...
  private void onCancel() {
    withSelected(
        item -> {
          appState.auctionService.cancelAuction(item.getId(), appState.currentUser);
          refreshTable();
          showStatus("Auction cancelled.", false);
        });
  }
  ...
  private void onDelete() {
    withSelected(
        item -> {
          appState.itemService.deleteItem(appState.currentUser, item.getId());
          refreshTable();
  ...
  private void onPlaceBid() {
    ...
    if (selected.getStatus() != AuctionStatus.RUNNING) {
      showStatus("Auction is not running.", true);
      return;
    }
          ...
          dlg.showAndWait()
              .ifPresent(
                  username -> {
                    try {
                      User updated =
                          appState.restUserService.changeUsername(user.getId(), username.trim());
  ...
  private void refreshUsers() {
    ...
    try {
      userTable.setItems(
          FXCollections.observableArrayList(appState.restUserService.findAllUsers()));
      userTable.refresh();
    } catch (Exception e) {
      showStatus(e.getMessage(), true);
    }
  ...
  private void handleRealtimeUpdate(String message) {
    ...
    try {
      ...
      if (event.has("type") && !event.get("type").isJsonNull())
        type = event.get("type").getAsString();
      if (event.has("userId") && !event.get("userId").isJsonNull())
  ...
  /**
   * Re-fetch the current user from the server after a balance-changing action (e.g. winning a bid +
   * auction settlement). Falls back silently if the server can't be reached so we don't blow up the
   * UI.
   */
  ...
  private void refreshUserInfo() {
    ...
    String balance = "";
    if (appState.currentUser instanceof Bidder b)
      balance = "  |  Balance: $" + String.format("%.2f", b.getBalance());
    if (appState.currentUser instanceof Seller s)
      balance = "  |  Balance: $" + String.format("%.2f", s.getBalance());
  ...
  private void showItemDetailsCard(Item item) {
    ...
    if (images.isEmpty()) {
      ...
      imageView.setOnMouseClicked(
          e -> ItemImageSupport.openImageViewer(images.get(imageIndex[0]), stylesheetUrl()));
      renderImage.run();
    ...
    r =
        addDetail(
            auction, r, "Starting price", "$" + String.format("%,.2f", item.getStartingPrice()));
    ...
    r =
        addDetail(
            ...
            r,
            "Start time",
            item.getStatus() != AuctionStatus.OPEN && item.getBidStartTime() != null
                ? item.getBidStartTime().format(DT_FMT)
                : "Not started");
    r =
        addDetail(
            auction,
            r,
            "End bid time",
            item.getStatus() != AuctionStatus.OPEN && item.getBidEndTime() != null
                ? item.getBidEndTime().format(DT_FMT)
                : "Not scheduled");
    ...
    if (item instanceof Art a) {
      ...
      s =
          addDetail(
              ...
              s,
              "Mfg date",
              v.getManufacturingDate() != null ? v.getManufacturingDate().toString() : "N/A");
    ...
    VBox info =
        new VBox(
            12, title, price, description, section("Auction", auction), section("Details", specs));
