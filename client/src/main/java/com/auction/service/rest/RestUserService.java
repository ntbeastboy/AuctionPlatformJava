...
/**
 * Network-backed UserService. login/register hit the REST API and pick
 * up the JWT bearer token for subsequent requests; balance helpers post
 * to /users/{id}/balance/{add,deduct} and return the refreshed user.
 *
 * Inherits from UserService only so it slots into the existing AppState
 * without any controller changes — none of the inherited methods are
 * actually invoked because every method below is overridden or new.
 */
public class RestUserService extends UserService {
    ...
    /**
     * Adds funds via the server and returns the refreshed user (with the
     * canonical balance from the database).
     */
    ...
    public User banUser(String userId, long durationSeconds) {
        try {
            String response = http.post("/users/" + userId + "/ban?durationSeconds=" + durationSeconds, "{}");
            Map<String, Object> map = http.getGson().fromJson(response, MAP);
            return JsonMappers.toUser(map);
        } catch (IOException e) {
            throw new UnauthorizedActionException(extract(e));
