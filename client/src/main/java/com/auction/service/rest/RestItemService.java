 ...
 *
 * <p>Only the two methods that mutate state are overridden; everything else stays inherited (there
 * is nothing else on the parent today).
 */
public class RestItemService extends ItemService {
  ...
  private Map<String, Object> itemToBody(Item item) {
    ...
    if (item.getBidStartTime() != null && item.getBidEndTime() != null) {
      mins =
          Math.max(1, Duration.between(item.getBidStartTime(), item.getBidEndTime()).toMinutes());
