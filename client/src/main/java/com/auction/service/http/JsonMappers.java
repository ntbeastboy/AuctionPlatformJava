...
/**
 * Translates JSON maps returned by the REST API into domain model instances used by the desktop
 * client.
 *
 * <p>password - the client never needs it (auth is already established by the JWT bearer token).
 * password - the client never needs it (auth is already established by the JWT bearer token).
 */
