...
public class HttpClientService {
  ...
  public String get(String endpoint) throws IOException {
    Request.Builder builder = new Request.Builder().url(url(endpoint));
    addAuth(builder);
  ...
  public String post(String endpoint, String jsonBody) throws IOException {
    ...
    Request.Builder builder = new Request.Builder().url(url(endpoint)).post(body);
    addAuth(builder);
    return execute(builder.build());
  }
  ...
  public String put(String endpoint, String jsonBody) throws IOException {
    ...
    Request.Builder builder = new Request.Builder().url(url(endpoint)).put(body);
    addAuth(builder);
    return execute(builder.build());
  }

  public String delete(String endpoint) throws IOException {
    Request.Builder builder = new Request.Builder().url(url(endpoint)).delete();
    addAuth(builder);
    return execute(builder.build());
  }

  /**
   * Runs the request and translates any failure into a user-friendly IOException whose message can
   * be shown verbatim in the UI.
   *
   * <p>- Connection failures → "Cannot reach the server. Please make sure the auction server is
   * running." - Server-rendered errors (JSON {"error": "..."}) → just the inner error string, no
   * "HTTP 500:" prefix. - Anything else → a short generic message.
   */
  private String execute(Request request) throws IOException {
    ...
    try {
      ...
      throw new IOException(
          "Cannot reach the server. Please make sure the auction server is running.");
