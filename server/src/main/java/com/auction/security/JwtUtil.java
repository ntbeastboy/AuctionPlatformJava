...
public class JwtUtil {
  ...
  public static Map<String, String> validateToken(String token) {
    if (token == null || token.isBlank()) throw new UnauthorizedActionException("Missing token.");

    String[] parts = token.split("\\.");
    if (parts.length != 3) throw new UnauthorizedActionException("Malformed token.");
