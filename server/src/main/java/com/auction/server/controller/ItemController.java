...
public class ItemController {
  ...
  public void handleGetAvailableItems(Context ctx) {
    List<Item> available =
        itemRepo.findAll().stream()
            .filter(
                i -> i.getStatus() == AuctionStatus.RUNNING || i.getStatus() == AuctionStatus.OPEN)
            .toList();
  ...
  public void handleGetSellerItems(Context ctx) {
    ...
    List<Item> items =
        itemRepo.findAll().stream().filter(i -> i.getSellerId().equals(sellerId)).toList();
  ...
  public void handleCreateItem(Context ctx) {
    ...
    Item item =
        itemFactory.create(
            type, id, name, description, startPrice, priceStep, start, end, sellerId, body);
  ...
  public void handleUpdateItem(Context ctx) {
    ...
    LocalDateTime start =
        existing.getBidStartTime() != null ? existing.getBidStartTime() : LocalDateTime.now();
    LocalDateTime end = start.plusMinutes(durationMinutes);
  ...
  private int durationMinutes(Item item) {
    ...
    return Math.max(
        1,
        (int) java.time.Duration.between(item.getBidStartTime(), item.getBidEndTime()).toMinutes());
  }
