...
public class SqliteAutoBidRepository implements AutoBidRepository {
    ...
    String sql =
        """
            INSERT INTO auto_bids (
                user_id, item_id, max_bid, increment, created_at, last_bid_at, next_check_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?)
  ...
  public List<AutoBid> findByItemId(String itemId) {
    String sql =
        """
            ...
            WHERE item_id = ?
            ORDER BY next_check_at ASC, max_bid DESC, rowid ASC
        """;
  ...
  public void recordBid(String userId, String itemId, long bidAt, long nextCheckAt) {
    String sql =
        "UPDATE auto_bids SET last_bid_at = ?, next_check_at = ? WHERE user_id = ? AND item_id = ?";
    try (PreparedStatement ps = conn.prepareStatement(sql)) {
      ps.setLong(1, bidAt);
