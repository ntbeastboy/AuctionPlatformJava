...
class AuctionServiceTest {
    ...
    @Test
    void closeAuctionWithSolventWinnerSettlesAsPaid() {
    ...
    void adminCanEndAuctionEarlyBeforeEndTime() {
        ...
        try {
            ...
            Item item = new Other("item", "Item", "Desc", 100.0, 5.0,
            ...
            assertEquals(AuctionStatus.PAID, items.findById(item.getId()).orElseThrow().getStatus());
        } finally {
            service.shutdown();
        }
    }
    ...
    private Item runningItem(String sellerId) {
        Item item = new Other("item", "Item", "Desc", 100.0, 5.0,
                LocalDateTime.now().minusMinutes(2), LocalDateTime.now().minusMinutes(1), sellerId);
        item.setStatus(AuctionStatus.RUNNING);
        return item;
    }
