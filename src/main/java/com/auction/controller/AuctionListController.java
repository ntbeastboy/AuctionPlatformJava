...
public class AuctionListController {
    ...
    @FXML private Button confirmPayBtn;
    ...
    private void configureRoleButtons() {
        if (appState.currentUser instanceof Admin) {
            show(startBtn, endEarlyBtn, cancelBtn, deleteBtn, markPaidBtn);
        } else if (appState.currentUser instanceof Seller) {
            show(addFundsBtn, withdrawBtn, createItemBtn, startBtn, placeBidBtn, confirmPayBtn);
        } else if (appState.currentUser instanceof Bidder) {
            show(addFundsBtn, withdrawBtn, placeBidBtn, confirmPayBtn);
        }
    }
    ...
    private void onConfirmPay() {
        withSelected(item -> {
            if (item.getStatus() != AuctionStatus.FINISHED) {
                throw new IllegalStateException("Auction must be FINISHED before confirming payment.");
            }
            if (!appState.currentUser.getId().equals(item.getCurrentWinnerId())) {
                throw new IllegalStateException("Only the winner can confirm this payment.");
            }
